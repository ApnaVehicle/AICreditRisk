    ...
    for (const loan of highRiskLoans) {
      ...
      if (latestRepayment && latestRepayment.dpd > 30) {
        alerts.push({
          ...
          alert_type: 'HIGH_DPD',
          priority: latestRepayment.dpd >= 90 ? 'critical' : latestRepayment.dpd > 60 ? 'high' : 'medium',
      ...
      if (assessment.risk_score > 75) {
        alerts.push({
          ...
          priority: assessment.risk_score >= 80 ? 'critical' : 'high',
      ...
      if (loan.status === 'RESTRUCTURED' || assessment.risk_category === 'HIGH') {
        if (latestRepayment && latestRepayment.dpd > 60) {
          alerts.push({
            ...
            customer_name: loan.customer.customer_name,
            alert_type: 'NPA_RISK',
            priority: latestRepayment.dpd >= 90 || loan.status === 'NPA' ? 'critical' : 'high',
